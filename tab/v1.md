...
Every compositor opens a single `AF_UNIX` `SOCK_STREAM` connection to `/tmp/shift.sock` and keeps it alive for the compositor’s lifetime. The socket carries both directions of traffic — Shift may send messages at any time (monitor hotplug, input, session updates, errors).
...
```ts
type MonitorRemovedPayload = { monitor: MonitorInfo };
```
...
session_created
{"session":{"id":"se_user2","role":"session","state":"pending","is_active":false},"token":"ses_QneY60mnZsYt2c9RgK+1CA=="}
...
session_state
{"session":{"id":"se_user2","role":"session","display_name":"alice","state":"occupied","is_active":false}}

```

## Session Control (session_switch, session_active) (session_switch, session_active)
